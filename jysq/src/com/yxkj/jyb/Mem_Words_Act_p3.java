...
public class Mem_Words_Act_p3 extends Activity{
	
	private String[][] memContents = null;
	private String[][] ansContents = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {  
        ...
        setContentView(R.layout.mem_words_act_p3);
        
        memContents = Mem_Words_Act_p1.memContents;
        ansContents = Mem_Words_Act_p2.ansContents;
        if(memContents == null || ansContents == null)
        	return;
        if(memContents.length != ansContents.length)
        	return;
        
        findViewById(R.id.back).setOnClickListener(new OnClickListener(){
			public void onClick(View v) {
				Mem_Words_Act_p3.this.finish();
			}  
		});
	...
	private void randomNumbers(int count){
		curPageCount = count / Mem_Words_Act_Main.oneGroupCount;
	}
	private void initViewList(){
		
		LayoutInflater lf = LayoutInflater.from(this);  
		for(int i=0;i<curPageCount;i++){
			View _v = lf.inflate(R.layout.mem_words_act_p3_page, null);  
			mViewList.add(_v);
		}
	...
	private void initPage(int _page, View _view){
		...
		for(int i=0;i<onecount;i++){
			((TextView)numbers.get(i)).setText(memContents[_page][i].toString());
		}
		//我的答案
		for(int i=0;i<onecount;i++){
			TextView tv = ((TextView)numbers.get(i+onecount));
			tv.setText(ansContents[_page][i]);
			tv.setTextColor(ansContents[_page][i].endsWith(memContents[_page][i]) ? 
	...
	static public void show(Context _c ){
		if(_c != null){
			...
			Intent intent = new Intent(_c, Mem_Words_Act_p3.class);
