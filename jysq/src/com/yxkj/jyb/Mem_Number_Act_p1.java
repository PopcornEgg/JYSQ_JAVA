...
public class Mem_Number_Act_p1 extends Activity{
	
	static public Integer[][] memNumbers = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {  
        ...
        setContentView(R.layout.mem_number_act_p1);
        
        findViewById(R.id.back).setOnClickListener(new OnClickListener(){
			public void onClick(View v) {
				Mem_Number_Act_p1.this.finish();
			}  
		});
		
        findViewById(R.id.ok).setOnClickListener(new OnClickListener(){
			public void onClick(View v) {
				Mem_Number_Act_p2.show(Mem_Number_Act_p1.this);
				Mem_Number_Act_p1.this.finish();
			}  
		}); 
	...
	private void randomNumbers(int count){
		...
		memNumbers = new Integer[curGroupCount][Mem_Number_Act_Main.oneGroupCount];
		for(int i=0;i<curGroupCount;i++){
			for(int j=0;j<Mem_Number_Act_Main.oneGroupCount;j++){
				memNumbers[i][j] = new Random().nextInt(10);
			}
		}
	...
	private void initViewList(){
		...
		for(int i=0;i<curPageCount;i++){
			View _v = lf.inflate(R.layout.mem_number_act_p1_page, null);  
			mViewList.add(_v);
		}
	...
	private void initPage(int _page, View _view){
		...
		for(int i=0;i<Mem_Number_Act_Main.oneGroupCount;i++){
			((TextView)numbers.get(i)).setText(memNumbers[idx][i].toString());
		}
		...
		if(curGroupCount > idx){
			...
			for(int i=0;i<Mem_Number_Act_Main.oneGroupCount;i++){
				((TextView)numbers.get(i + Mem_Number_Act_Main.oneGroupCount)).setText(memNumbers[idx][i].toString());
			}
		}else{
	...
	static public void show(Context _c ){
		if(_c != null){
			...
			Intent intent = new Intent(_c, Mem_Number_Act_p1.class);
